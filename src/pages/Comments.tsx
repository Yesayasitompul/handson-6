...
// Defines a TypeScript interface for the Comments object, specifying that it contains an array of Comment objects.

interface DeletedComment extends Comment {
  isDeleted: Boolean;
  deletedOn: string;
}
// Defines a TypeScript interface for the DeletedComment object, extending the Comment interface and adding isDeleted and deletedOn properties.

const fetchCommentsDat = async () => {
  return axios.get<Comments>('/comments');
};
// Defines an asynchronous function fetchCommentsDat that sends a GET request to fetch comments data.

const deleteComments = async (id: string | undefined) => {
  return await axios.delete<DeletedComment>(`comments/${id}`);
};
// Defines an asynchronous function deleteComments that takes an id as input and sends a DELETE request to delete the comment.
...
// Defines a functional component CommentsSkeleton that renders a skeleton loader for comments.
...
const Comments = () => {
  ...
  // Uses the useQuery hook to fetch comments data. The queryKey is "comments" and the query function is fetchCommentsDat.

  const deleteCommentDat = useMutation({
    mutationFn: (id: string) => deleteComments(id)
  });
  // Uses the useMutation hook to create a mutation for deleting a comment. The mutation function is deleteComments.
  ...
  // Renders the Comments component. If the comments data is being fetched, it shows skeleton loaders. Otherwise, it maps over the comments data and renders each comment.
}
