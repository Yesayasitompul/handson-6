...
// Defines a TypeScript interface for the TodoList object, specifying that it contains an array of Todo objects.

interface DeletedTodo extends Todo {
    isDeleted: Boolean;
    deletedOn: string;
}
// Defines a TypeScript interface for the DeletedTodo object, extending Todo and adding isDeleted and deletedOn properties.

const fetchTodoList = async () => {
    return axios.get<TodoList>('/todo');
};
// Defines an asynchronous function fetchTodoList that sends a GET request to fetch the list of todos.

const deleteTodo = async (id: string | undefined) => {
    return await axios.delete<DeletedTodo>(`todo/${id}`);
};
// Defines an asynchronous function deleteTodo that takes an id as input and sends a DELETE request to delete the todo.
...
// Defines a functional component TodoSkeleton that renders a skeleton loader for todos while data is being fetched.
...
const Todo = () => {
    ...
    // Uses the useQuery hook to fetch the list of todos. The queryKey is "Todo" and the query function is fetchTodoList.

    const deleteTodoMutation = useMutation({
        mutationFn: (id: string) => deleteTodo(id)
    });
    // Uses the useMutation hook to create a mutation for deleting a todo. The mutation function is deleteTodo.
    ...
    return (
        <div className="container mx-auto px-4 bg-gray-900 min-h-screen">
            ...
            {/* Renders a button that navigates to the "Add Todo" page when clicked. */}
    ...
    // Renders the Todo component. If the todo list is being fetched, it shows skeleton loaders. Otherwise, it displays the list of todos. Each todo card includes options to edit or delete the todo.
}

export default Todo;
// Exports the Todo component as the default export.